...
class Storage {
  ...
  Future<void> uploadDocument(String? filePath, String fileName) async {
    ...
    String? email = pref.getString("email");
    try {
      await storage.ref('Documents/$email/$fileName').putFile(File(filePath!));
    } on firebase_core.FirebaseException catch (e) {
      print(e);
    }
  }
  ...
  Future<firebase_storage.ListResult> listFiles() async {
    ...
    firebase_storage.ListResult results =
        await storage.ref('$email/$listingNo/').listAll();
  ...
  Future<String> downloadImageURL(String imageName) async {
    ...
    String? email = pref.getString("email");
    String downloadURL =
        await storage.ref('$email/$listingNo/$imageName').getDownloadURL();
    return downloadURL;
  }
  ...
  Future<String> downloadPDFURL(String fileName) async {
    ...
    String? email = pref.getString("email");
    String downloadURL =
        await storage.ref('Billings/$email/$fileName').getDownloadURL();
    return downloadURL;
  }

  Future<String> downloadDocumentLink(String fileName) async {
    final pref = await SharedPreferences.getInstance();
    String? email = pref.getString("email");
    String downloadURL =
        await storage.ref('Documents/$email/$fileName').getDownloadURL();
    return downloadURL;
  }
}
